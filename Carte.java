...
public class Carte {
    ...
    public void vadeRetroShadock(Graphe g){
        int sha = 0;//sommet inexistant
        for (int i = 1; i<= this.getTaille(); i++){
            for (int j = 1;  j<= this.getTaille(); j++){
                if (this.getCase(i, j).getVaisseau().getRace() == this.getCase(i, j).getVaisseau().getRace().Shadoks){
                    sha =(i - 1) * this.getTaille() + j;
                }
            }
        }
        for (int s= 1; s<=this.getGraphe().getNbSommet(); s++){
            if(g.getMatrice(sha, s)!=0){
                g.modifierMatrice(sha, s, 0);               
            }
        }
    ...
    /**
     * Donne les coordonées d'un numéro de sommet sous la forme d'un couple (X,Y)
